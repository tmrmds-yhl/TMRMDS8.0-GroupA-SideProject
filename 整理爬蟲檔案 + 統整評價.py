...
path_list = []
for i in range(10):
    path_list.append("C:\\Users\\Jye-li\\OneDrive\\桌面\\Course_info\\" + course_name_list[i] + ".csv")

course_total = pd.read_csv("C:\\Users\\Jye-li\\OneDrive\\桌面\\Course_info\\course_total.csv")
list_of_dict = []
...
course_total.to_csv("C:\\Users\\Jye-li\\OneDrive\\桌面\\Course_info\\course_total.csv")
...
# API key
openai.api_key = "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"

# 重新讀入 csv
course_total_api = pd.read_csv("C:\\Users\\Jye-li\\OneDrive\\桌面\\Course_info\\course_total.csv")
...
for i in range(len(course_total_api)):
    ...
    response = openai.ChatCompletion.create(
        ...
        temperature=0.5,
        messages=[
            {"role": "user", "content": "下列清單當中，每一項元素為一個課程評價，請將下列的課程評論內容，統整成3個重點，且須最真實反映課程評價: "}, 
...
course_total_api.insert(len(course_total_api.columns), column = "評價標題的結論", value = comment_title_summary_total)
...
course_total_api.insert(len(course_total_api.columns), column = "評價內容的結論", value = comment_content_summary_total)

# 下面這個要跑，才會更新 csv 檔
course_total_api.to_csv("C:\\Users\\Jye-li\\OneDrive\\桌面\\Course_info\\course_total.csv")
