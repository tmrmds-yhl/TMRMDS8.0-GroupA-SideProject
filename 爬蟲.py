...
def collect_comments(browser, number, coursename):
    # 點選第一堂課
    WebDriverWait(browser, 30).until(EC.presence_of_element_located((By.CLASS_NAME, 'title.marg-t-20.marg-b-10')))
    #search_click = browser.find_elements_by_class_name('title.marg-t-20.marg-b-10')[0]
    search_click = browser.find_elements_by_class_name('title.marg-t-20.marg-b-10')[number]
    search_click.click()

    # 點選所有評論
    WebDriverWait(browser, 30).until(EC.presence_of_element_located((By.CLASS_NAME, 'sc-hvnn7l-2.bcCbbE')))
    ...
    while True:
        try:
            WebDriverWait(browser, 30).until(EC.presence_of_element_located((By.CLASS_NAME, 'sc-1a6j6ze-0.cYdxxq.sc-b21euj-2.buAXqm')))
            search_click = browser.find_element_by_class_name('sc-1a6j6ze-0.cYdxxq.sc-b21euj-2.buAXqm')
            search_click.click()
        except:
            break
...
def course_searching(search):
    ...
    # 輸入關鍵字
    WebDriverWait(browser, 30).until(EC.presence_of_element_located((By.CLASS_NAME, 'sc-3izbpp-3.fVBoBi')))
    search_input = browser.find_elements_by_class_name('sc-3izbpp-3.fVBoBi')[0]
    search_input.send_keys(search)

    # 點選搜尋鍵
    WebDriverWait(browser, 30).until(EC.presence_of_element_located((By.CLASS_NAME, 'sc-3izbpp-0.cwlbXJ')))
    search_click = browser.find_elements_by_class_name('sc-3izbpp-0.cwlbXJ')[0]
    search_click.click()

    # 點選影音課程
    WebDriverWait(browser, 30).until(EC.presence_of_element_located((By.CLASS_NAME, 'sc-v1wcho-1.bqEVBe')))
